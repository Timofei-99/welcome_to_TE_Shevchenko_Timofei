...
(async () => {
	const filteredPublickBallons = Object.values(BALLONS).filter((ballon) => ballon.isPublic);

	const amount = await Promise.all(filteredPublickBallons.map(ballon => fetchBallonAmount(ballon.id)));
